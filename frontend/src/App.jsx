...
function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!createResponse.ok) {
        throw new Error(
          `Could not create project (status ${createResponse.status})`,
        );
      }
      ...
      const generateResponse = await fetch(
        `${API_URL}/projects/${project.id}/generate`,
        { method: "POST" },
      );
  ...
  const handleGenerateDiagram = async (type) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/projects/${result.project.id}/diagrams/${type}`,
        { method: "POST" },
      );
  ...
  const openHistory = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(`Could not load projects (status ${res.status})`);
      const data = await res.json();
  ...
  const handleDeleteProject = async (e, projectId) => {
    ...
    if (
      !confirm(
        "Delete this project and all its generated content? This cannot be undone.",
      )
    ) {
      return;
    }
    ...
    <div className="App">
      ...
      {view === "history" && (
        <div className="history-list">
          {historyLoading && (
            <p className="agent-text">Loading past projects...</p>
          )}
